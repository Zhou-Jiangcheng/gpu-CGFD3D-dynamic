                   ...
                   int *topoid, iofault_nc_t *iofault_nc)
{
  ...
  if(num_of_fault == 1) 
  {
    ...
    if (nc_def_var(iofault_nc->ncid, "final_slip1", NC_FLOAT, 2, dimid+1, &(iofault_nc->varid[13])))  M_NCERR;   
    if (nc_def_var(iofault_nc->ncid, "final_slip2", NC_FLOAT, 2, dimid+1, &(iofault_nc->varid[14])))  M_NCERR;   
                      ...
                      float *buff)
{
  ...
  CUDACHECK(cudaMemcpy(buff,F.slip1,size,cudaMemcpyDeviceToHost));
  nc_put_var_float(iofault_nc->ncid, iofault_nc->varid[13], buff);

  CUDACHECK(cudaMemcpy(buff,F.slip2,size,cudaMemcpyDeviceToHost));
  nc_put_var_float(iofault_nc->ncid, iofault_nc->varid[14], buff);
  return ierr;
}
