    ...
    if(isfree == 1 && km==0) 
    {
      for (int i = 0; i < 3; i++){
        for (int j = 0; j < 3; j++){
          ...
          matT1toVxfm[i][j] = FC.matT1toVxfm[(iy+3)*9 + i*3 + j];
          matVytoVxfm[i][j] = FC.matVytoVxfm[(iy+3)*9 + i*3 + j];
          matT1toVxfp[i][j] = FC.matT1toVxfp[(iy+3)*9 + i*3 + j];
          matVytoVxfp[i][j] = FC.matVytoVxfp[(iy+3)*9 + i*3 + j];
        }
      }
