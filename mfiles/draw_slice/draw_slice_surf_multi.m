        ...
        if slicedir{i} == 'x'
            ...
            for kp=0:nprok-1
                ...
                for jp=0:nproj-1
                    ...
                    % snapshot data
                    slicestruct{i}=dir([output_dir,'/','slicex_i',num2str(sliceid{i}),'_px*_py',num2str(jp),'_pz',num2str(kp),'.nc']);
                    ...
                    if jp==0
                        VV{i}=squeeze(nc_varget(slicenm{i},varnm,[nlayer-1,0,0],[1,pnk{i},pnj{i}],[1,1,1]));
                    else
                        VV0{i}=squeeze(nc_varget(slicenm{i},varnm,[nlayer-1,0,0],[1,pnk{i},pnj{i}],[1,1,1]));
                    ...
                    slicenm{i}=[output_dir,'/',slicenm{i}];
                    coordnm{i}=['coord','_px',num2str(ip),'_py',num2str(jp),'_pz',num2str(kp),'.nc'];
                   ...
                   if jp==0
                        XX{i}=squeeze(nc_varget(coordnm{i},'x',[ghostp,ghostp,idwithghost],[pnk{i},pnj{i},1],[1,1,1]));
                        YY{i}=squeeze(nc_varget(coordnm{i},'y',[ghostp,ghostp,idwithghost],[pnk{i},pnj{i},1],[1,1,1]));
                        ZZ{i}=squeeze(nc_varget(coordnm{i},'z',[ghostp,ghostp,idwithghost],[pnk{i},pnj{i},1],[1,1,1]));
                    else
                        XX0{i}=squeeze(nc_varget(coordnm{i},'x',[ghostp,ghostp,idwithghost],[pnk{i},pnj{i},1],[1,1,1]));
                        YY0{i}=squeeze(nc_varget(coordnm{i},'y',[ghostp,ghostp,idwithghost],[pnk{i},pnj{i},1],[1,1,1]));
                        ZZ0{i}=squeeze(nc_varget(coordnm{i},'z',[ghostp,ghostp,idwithghost],[pnk{i},pnj{i},1],[1,1,1]));
        ...
        elseif slicedir{i} == 'y'
            ...
            for kp=0:nprok-1
                ...
                for ip=0:nproi-1
                    ...
                    % snapshot data
                    slicestruct{i}=dir([output_dir,'/','slicey_j',num2str(sliceid{i}),'_px',num2str(ip),'_py*','_pz',num2str(kp),'.nc']);
                    slicenm{i}=slicestruct{i}.name;
                    slicenm{i}=[output_dir,'/',slicenm{i}];
                    ...
                    pnk{i}=pnkstruct{i}.Length;
                    if ip==0
                        VV{i}=squeeze(nc_varget(slicenm{i},varnm,[nlayer-1,0,0],[1,pnk{i},pni{i}],[1,1,1]));
                    else
                        VV0{i}=squeeze(nc_varget(slicenm{i},varnm,[nlayer-1,0,0],[1,pnk{i},pni{i}],[1,1,1]));
                        VV{i}=vertcat(VV{i},VV0{i});
                    end
                    t=nc_varget(slicenm{i},'time',[nlayer-1],[1]);
                    
                    % coordinate data
                    slicenm{i}=slicestruct{i}.name;
                    jp=str2num(slicenm{i}( strfind(slicenm{i},'py')+2 : strfind(slicenm{i},'_pz')-1 ));
                    slicenm{i}=[output_dir,'/',slicenm{i}];
                    coordnm{i}=['coord','_px',num2str(ip),'_py',num2str(jp),'_pz',num2str(kp),'.nc'];
                    coordnm{i}=[output_dir,'/',coordnm{i}];
                    ...
                    ghostp=(coorddimstruct{i}.Length-slicedimstruct{i}.Length)/2;
                    if ip==0
                        XX{i}=squeeze(nc_varget(coordnm{i},'x',[ghostp,idwithghost,ghostp],[pnk{i},1,pni{i}],[1,1,1]));
                        YY{i}=squeeze(nc_varget(coordnm{i},'y',[ghostp,idwithghost,ghostp],[pnk{i},1,pni{i}],[1,1,1]));
                        ZZ{i}=squeeze(nc_varget(coordnm{i},'z',[ghostp,idwithghost,ghostp],[pnk{i},1,pni{i}],[1,1,1]));
                    else
                        XX0{i}=squeeze(nc_varget(coordnm{i},'x',[ghostp,idwithghost,ghostp],[pnk{i},1,pni{i}],[1,1,1]));
                        YY0{i}=squeeze(nc_varget(coordnm{i},'y',[ghostp,idwithghost,ghostp],[pnk{i},1,pni{i}],[1,1,1]));
                        ZZ0{i}=squeeze(nc_varget(coordnm{i},'z',[ghostp,idwithghost,ghostp],[pnk{i},1,pni{i}],[1,1,1]));
                        XX{i}=vertcat(XX{i},XX0{i});
                        YY{i}=vertcat(YY{i},YY0{i});
                        ZZ{i}=vertcat(ZZ{i},ZZ0{i});
                    end             
                end
                if kp==0
                    V{i}=VV{i};
...
%             view(45,15);
            ...
            for jp=0:nproj-1
                ...
                for ip=0:nproi-1
                    ...
                    % snapshot data
                    slicestruct{i}=dir([output_dir,'/','slicez_k',num2str(sliceid{i}),'_px',num2str(ip),'_py',num2str(jp),'_pz*.nc']);
                    slicenm{i}=slicestruct{i}.name;
                    slicenm{i}=[output_dir,'/',slicenm{i}];
                    pnistruct{i}=nc_getdiminfo(slicenm{i},'i');
                    pni{i}=pnistruct{i}.Length;
                    ...
                    % coordinate data
                    slicenm{i}=slicestruct{i}.name;
                    kp=str2num(slicenm{i}( strfind(slicenm{i},'pz')+2 : strfind(slicenm{i},'.nc')-1 ));
                    slicenm{i}=[output_dir,'/',slicenm{i}];
                    coordnm{i}=['coord','_px',num2str(ip),'_py',num2str(jp),'_pz',num2str(kp),'.nc'];
                    coordnm{i}=[output_dir,'/',coordnm{i}];
