function [V, X, Y, Z, t] = gather_slice_y(output_dir,nlayer,varnm,sliceid,nproi,nprok)
  ...
  for kp=0:nprok-1
    for ip=0:nproi-1
      ...
      idwithghost=double(nc_attget(slicenm_dir,nc_global,'j_index_with_ghosts_in_this_thread'));
      ...
      if ip==0
        XX=squeeze(nc_varget(coordnm_dir,'x',[ghostp,idwithghost,ghostp],[pnk,1,pni],[1,1,1]));
        YY=squeeze(nc_varget(coordnm_dir,'y',[ghostp,idwithghost,ghostp],[pnk,1,pni],[1,1,1]));
        ZZ=squeeze(nc_varget(coordnm_dir,'z',[ghostp,idwithghost,ghostp],[pnk,1,pni],[1,1,1]));
      else
        XX0=squeeze(nc_varget(coordnm_dir,'x',[ghostp,idwithghost,ghostp],[pnk,1,pni],[1,1,1]));
        YY0=squeeze(nc_varget(coordnm_dir,'y',[ghostp,idwithghost,ghostp],[pnk,1,pni],[1,1,1]));
        ZZ0=squeeze(nc_varget(coordnm_dir,'z',[ghostp,idwithghost,ghostp],[pnk,1,pni],[1,1,1]));
        XX=vertcat(XX,XX0);
        YY=vertcat(YY,YY0);
        ZZ=vertcat(ZZ,ZZ0);
      end  
