...
int init_fault_device(gdinfo_t *gdinfo, fault_t *F, fault_t *F_d)
{
  ...
  F_d->mslip      = (float *) cuda_malloc(sizeof(float)*nj*nk);
  F_d->tslip      = (float *) cuda_malloc(sizeof(float)*nj*nk);
  ...
  CUDACHECK( cudaMemcpy(F_d->mslip, F->mslip,  sizeof(float)*nj*nk, cudaMemcpyHostToDevice) );
  CUDACHECK( cudaMemcpy(F_d->tslip, F->tslip,  sizeof(float)*nj*nk, cudaMemcpyHostToDevice) );
