...
__global__ void
rk_update(int istage, int it, float dt, float rka, float rkb, float *f_cur_d,
          gdinfo_t gdinfo_d, fault_t F, fault_coef_t FC, fault_wav_t FW)
{
  size_t iy = blockIdx.x * blockDim.x + threadIdx.x;
  size_t iz = blockIdx.y * blockDim.y + threadIdx.y;
  int nj = gdinfo_d.nj;
  ...
  if(iy < nj && iz < nk && F.united[iptr_t]==0)
  {
    ...
    if(istage == 0)
    {
      F.tslip[iptr_t] = F.mslip[iptr_t] + rkb * F.hslip[iptr_t];
    } else if(istage == 1 || istage ==2) {
      F.tslip[iptr_t] +=  rkb * F.hslip[iptr_t];
    }

    if(istage == 3)
    {
      F.slip[iptr_t] = F.tslip[iptr_t] + rkb * F.hslip[iptr_t];
      F.mslip[iptr_t] = F.slip[iptr_t];
      F.slip1[iptr_t] = Vs1 * dt;
      F.slip2[iptr_t] = Vs2 * dt;
    }else{ 
      F.slip[iptr_t] = F.mslip[iptr_t] + rka * F.hslip[iptr_t];
    }
